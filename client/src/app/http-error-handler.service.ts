import { Injectable } from '@angular/core';
import { HttpErrorResponse } from '@angular/common/http';

import { Observable } from 'rxjs/Observable';
import { of } from 'rxjs/observable/of';

/*

  Type of the handleError function returned by HttpErrorHandler.createHandleError

*/
export type HandleError = <T> (operation?: string, result?: T) =>
  (error: HttpErrorResponse) => Observable<T>;

/*

  Handles HttpClient errors

*/
@Injectable()
export class HttpErrorHandler {

  constructor() { }

  /*

    Create curried handleError function that already knows the service name

  */
  createHandleError = (serviceName = '') => <T>
    (operation = 'operation', result = {} as T) => this.handleError(serviceName, operation, result);

  /*

    Returns a function that handles Http operation failures.
    This error handler lets the app continue to run as if no error occurred.

      serviceName: name of the data service that attempted the operation
      operation: name of the operation that failed
      result: optional value to return as the observable result

  */
  handleError<T> (serviceName = '', operation = 'operation', result = {} as T)
  {

    return (error: HttpErrorResponse): Observable<T> => {

      // TODO: send to sentry
      // console.error(error);

      const message = (error.error instanceof ErrorEvent) ? error.error.message :
       `server returned code ${error.status} with body "${error.error}"`;

      // TODO: notify app of error
      // this.someService.notify(`${serviceName}: ${operation} failed: ${message}`);

      console.log(`${serviceName}: ${operation} failed: ${message}`);

      // let app keep running by returning a safe result
      return of(result);

    };

  }

}
